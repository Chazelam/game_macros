...
steam_games = {
    ...
    "Elden_Ring":    {"game_id": "1245620", "account": "kz", "account_id": "76561199728650271"},
...
def steam_login(acc: str, re_login: bool = True, attempts: int = 10):
...
def start_elden_ring_moded():
    pass

def launch_game(game_name: str):
    if game_name not in steam_games:
        print(f"Игра {game_name} не найдена в словаре!")
        return

    game = steam_games[game_name]
    current_acc = get_current_steam_account()

    if current_acc != game["account_id"]:
        print(f"Нужен аккаунт {game['account']}, переключаем...")
        success = steam_login(game["account"])
        if not success:
    ...
    if game["game_id"] == "1245620":
        start_elden_ring_moded()
    else:
        start_game(game["game_id"])
